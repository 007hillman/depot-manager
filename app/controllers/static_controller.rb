class StaticController < ApplicationController
  ...
  def receipt
    ...
    pdf.font "Courier"
    pdf.image "#{Rails.root}/app/assets/images/logo.png" , :at => [20, 720], :scale => 0.20 
    pdf.draw_text "Receipt: ", :at => [15,610], :size => 14, style: :bold
    pdf.font "Courier", size: 10
    pdf.bounding_box([15,600], :width => 400) do
    ...
    pdf.bounding_box([15,540], :width => 400) do
      pdf.text('Client : ' + @command.client_name)
      pdf.text('Location: ')
      pdf.text('Tel : ')
      pdf.text('Email: ')
    end
    pdf.draw_text '-------------------------------', at: [10,478]
    pdf.draw_text '-------------------------------', at: [10, 480]
    pdf.draw_text 'Items: ', style: :bold, size: 14, at: [15, 460]
    pdf.draw_text 'Code    |Qty  |UP    |TP', at: [15, 445]

    pdf.bounding_box([15, 435], width: 400) do
      @command.items.each do |item|
        total_price = item.quantity * item.drink.wholesale_price
        sum_total += total_price
        if(item.drink.supplyer == "Brasseries du Cameroun" and item.drink.packaging == "crate")
          crates_needed += item.quantity
        end
        if(item.drink.supplyer == "Guinness Cameroun SA" and item.drink.packaging == "crate")
          g_crates_needed += item.quantity
        end

        pdf.text item.drink.abbreviation + "| " + item.quantity.to_s + "| "+ item.drink.wholesale_price.to_s + "| " + total_price.to_s
      end
      pdf.text 'TOTAL:                 ' + sum_total.to_s, style: :bold
      pdf.text 'Amount paid :           ' + @command.amount_paid.to_s
    end
    pdf.text "-----------------------------------"
